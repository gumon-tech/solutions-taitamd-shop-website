...
**และอ่านใบสองใบนี้ก่อนเขียนใบแรก** (อยู่ที่ `~/dev/gumon-workspace/machines/komphet-mac/queue/ALL/`)

| ใบ | สิ่งที่คุม |
|---|---|
| `Q-WS-198` | **3 ทะเบียน** — คุยกับเจ้าของเป็นประโยคธรรมดาที่อ่านออกเสียงได้ · คุยกันเองระหว่างห้องย่อได้ · เนื้อหาที่คนนอกอ่านห้ามอีโมจิ<br>⚠️ **ข้อ 2 ของใบนี้ตายแล้ว ถูก `Q-WS-200` ทับ** อย่าทำตามข้อ 2 |
| `Q-WS-200` | **ฉบับที่ใช้จริงเรื่องอีโมจิแทนคำ** เกณฑ์เดียวคือ **อยู่ในช่อง หรืออยู่ในตำแหน่งคำ**<br>ลบออกแล้วเหลือช่องว่างที่เห็นว่าขาด แปลว่าแทนได้ · ลบแล้วประโยคยังอ่านรู้เรื่องแต่ความหมายเปลี่ยน แปลว่าแทนไม่ได้<br>**แทนได้** ป้ายนำหน้าบรรทัดหรือหัวข้อ · เซลล์ในคอลัมน์สถานะ · ป้ายห้ามที่มีคำว่าห้ามเขียนอยู่ด้วย<br>🔴 **แทนไม่ได้เด็ดขาด** คำปฏิเสธในประโยค (ไม่ ห้าม อย่า) · ต้อง ควร ได้ · คำตอบของคำถาม · คำเชื่อมเหตุผล · ตัวเลข<br>**ลูกศร** ทะเบียนในใช้เป็นตัวคั่นได้ แต่ห้ามเป็นคำเชื่อมที่แบกเหตุผลของประโยค |
| `Q-WS-209` | 🔴 **ขอบเขตจากเจ้าของ** งานคือ Taitam-D เท่านั้น และใน Taitam-D เอา **Platform กับคอร์ส A1 ก่อน**<br>ห้องนี้ **อยู่ในขอบเขต แต่เท่าที่งานรับใช้ Platform กับ A1** และไม่มี `_PAUSED.md`<br>📌 **ก่อนหยิบแถวไหน ต้องตอบให้ได้ว่ามันรับใช้ A1 หรือ Platform ตรงไหน** ตอบไม่ได้ แปลว่ารอ |
| `Q-WS-202` | **7 กฎประหยัดโทเคน** · ใบต้องมีบล็อกคำสั่งบนสุดไม่เกิน 15 บรรทัด · 🔴 **ห้ามตัด 5 อย่าง** คือ ตัวเลขพร้อมหน่วยและเวลาที่วัด · ชื่อไฟล์เต็ม · commit hash · คำสั่งที่ใช้วัด · ป้ายว่าเป็นผลวัดหรือข้อสันนิษฐาน |
| `Q-WS-204` | **เลือก model ของ Executor ตามชนิดงาน และแจ้งเหตุผลทุกครั้ง** ในใบคิวหรือบรรทัดสถานะ **ไม่ใช่ในหัว Lead** · เกณฑ์ย่อ: งานเชิงกลที่เกณฑ์เขียนครบแล้วใช้ `haiku` · แก้ไฟล์ตามทางแก้ที่เขียนไว้แล้วใช้ `sonnet` · **งานที่ผิดแล้วกลับความหมายหรือกลับคำตัดสิน ใช้รุ่นระดับ Lead** · บังคับตั้งแต่การแตกครั้งถัดไป ไม่ย้อนแก้ของเดิม<br>📌 **ต่อยอดจาก §1 ที่ห้องนี้รับมาแล้ว** — §1 คุม model ของ *ตัว Lead เอง* ส่วนใบนี้คุม model ของ *Executor ที่ Lead แตก* |
