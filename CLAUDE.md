...
### 🔑 บทบาท กับ รหัสห้อง เป็นคนละอย่าง — อ่านข้อนี้ก่อนประกาศตัว

```
บทบาท (พกพาไปกับรีโป)   ห้องนี้คือ Taitam-D Shop Website Lead
รหัสห้อง (ผูกกับเครื่อง)   อ่านจาก machines/$(cat ~/dev/gumon-workspace/.machine-id)/queue/README.md
                       ของรีโป gumon-workspace บนเครื่องที่คุณยืนอยู่ ที่เดียวเท่านั้น
ตารางอ้างอิง            SHOP @komphet-mac · KWEB @komphet-air   (อ้างอิง ไม่ใช่แหล่งความจริง)
```

🔴 **รีโปนี้เดินพร้อมกันได้หลายเครื่อง** ⇒ เอกสารในรีโปบอก **บทบาท** ได้ แต่บอก **รหัสห้องบนเครื่องนี้** ไม่ได้
🔑 ***ชื่อห้องเป็นของ เครื่อง+รีโป ไม่ใช่ของรีโป*** (กติกา `Q-KWS-004` ข้อ 3.1.3)
⚠️ เกิดจริง 2026-08-23: session ที่เปิดบน `komphet-air` อ่านเอกสารชุดนี้แล้วสวมรหัส `SHOP`
ของ `komphet-mac` ทั้งเทิร์น ทั้งที่ห้องนั้นยังเดินอยู่ **ห้องอื่นทักถึงรู้ตัว**

**ห้อง Taitam-D Shop Website Lead เป็นเจ้าของ repo นี้คนเดียว** — ห้องอื่นในกลุ่ม Taitam-D
...
🔴 **วัดด้วยการเปิดหน้าเปล่า ๆ ไม่พอ — ต้องวัดเคสที่มาจากโฆษณาด้วย** (เพิ่ม 2026-08-23)
```bash
# เปิดด้วย gclid แล้วกด Accept แล้วค่อยอ่าน — คุกกี้โฆษณาโผล่เฉพาะทางนี้
# https://taitam-d.com/?gclid=TEST  →  Accept  →  cookieStore.getAll()
```
`_gcl_aw` (90 วัน) **ขึ้นบน production ตั้งแต่แอดเริ่มยิง 5 ส.ค. แต่ทะเบียนเพิ่งมีแถวนี้ 23 ส.ค.** —
ตามหลังของจริง **15 วัน** เพราะทุกครั้งที่ตรวจ ตรวจด้วยการเปิดหน้าเปล่า ซึ่งไม่มีวันเห็นมัน
🔑 ***ด่านที่ตรวจด้วยเส้นทางที่ทีมใช้ จะไม่มีวันเห็นของที่โผล่เฉพาะเส้นทางที่ลูกค้าใช้***
🔴 **และอ่าน `localStorage` ด้วย ไม่ใช่แค่คุกกี้** (เพิ่ม 2026-08-23 หลังเจอตัวที่ 2)
`_gcl_ls` (90 วัน) เก็บ **ค่า `gclid` ไว้ใน `localStorage`** ⇒ `cookieStore.getAll()` มองไม่เห็นเลย
ตารางในหน้า `/cookies` ครอบ `localStorage` อยู่แล้วตั้งแต่แถว `ttd-consent` ⇒ ขาดแถวนี้คือขาดจริง
```bash
# ต้องอ่านทั้งสองที่ ที่เดียวไม่พอ
# cookieStore.getAll()   แล้ว   Object.keys(localStorage)
```
🔑 ***ด่านที่ตรวจเฉพาะที่เก็บของชนิดเดียว จะมองไม่เห็นของชนิดเดียวกันที่ถูกเก็บอีกที่***
📌 `_gcl_au` เห็น 1 ครั้งแล้ววัดซ้ำไม่เจอ ⇒ **จงใจไม่ใส่ในตาราง** เพราะตารางประกาศว่าครบและตรวจสอบได้
⇒ ใส่แถวที่ยืนยันไม่ได้ คือคำแถลงเท็จในทิศตรงข้าม · ถ้อยคำถ้าวัดเจอซ้ำ Legal ให้ไว้แล้วใน `Q-KWS-013`
...
**4. "Since 2009" ถูกต้องแล้ว ครูนกยืนยันเอง — เรื่องปิด ⛔ ห้ามเปิดใหม่**
...
🔴 **แก้หัวข้อนี้ 2026-09-09 (`Q-WS-454`) — ถ้อยคำเดิมเขียนว่า "ห้ามแตะจนกว่าเจ้าของจะเคาะ" ซึ่ง *ตกรุ่นไปแล้ว 1 เดือน***
เจ้าของเคาะไปแล้วตั้งแต่ **2026-08-09 คอมมิต `8dda4fd`** ครูนกยืนยันเองว่าเว็บถูก และหลักที่ปิดเรื่องคือ
**ปีก่อตั้ง ⛔ ไม่ใช่ปีจดทะเบียนบริษัท** ⇒ วันที่ใน Companies House จึงไม่เกี่ยวกับคำถามนี้ ⛔ ไม่ใช่หลักฐานที่ค้าน
โปสเตอร์หน้าร้าน `SINCE 2013` คือปีเปิดร้าน ⛔ ไม่ขัดกัน · `2009` ยืนทุกจุดรวม `foundingDate`

⚠️ **ราคาของถ้อยคำที่ตกรุ่น วัดได้แล้ว** — 2026-09-09 `MKT` อ่านข้อนี้แล้วสั่งใน `Q-MKT-077` ให้ถอด
`Established in 2009` ออกจาก meta description "จนกว่า D-71 จะปิด" ทั้งที่มันปิดไปแล้ว ·
ห้องนี้ปฏิเสธถูก แต่ปฏิเสธ**ด้วยเหตุผลที่ผิด** คืออ้างว่าเรื่องยังค้างรอเจ้าของ แล้วรายงานเจ้าของไปแบบนั้นด้วย
🔑 ***กฎที่เขียนว่า "รอคำตอบ" หลังจากคำตอบมาถึงแล้ว ⛔ ไม่ได้อ่านเหมือนกฎที่ล้าสมัย มันอ่านเหมือนงานที่ยังไม่เสร็จ
และทุกคนที่อ่านจะพยายามทำให้มันเสร็จ*** — `MKT` พยายามครั้งหนึ่ง ห้องนี้พยายามครั้งหนึ่งในวันเดียวกัน
...
| `queue-protocol.md` | เดินเลข **ในเขตรหัสห้องของเครื่องที่คุณยืนอยู่เท่านั้น** (`R` = รหัสจากตารางข้างบน) · ก่อน commit ใบใหม่รัน `grep -h "^id: Q-$R-" machines/*/queue/*/Q-$R-*.md \| sort \| uniq -c` **ทุกบรรทัดต้องเป็น 1** |

**และอ่านใบสองใบนี้ก่อนเขียนใบแรก** (อยู่ที่ `~/dev/gumon-workspace/machines/komphet-mac/queue/ALL/`)

📌 **ใบชุดนั้นอยู่ในตู้ของ `komphet-mac`** — ตู้ `ALL` ของเครื่องหนึ่งผูกพันเฉพาะห้องของเครื่องนั้น (`Q-TWS-001` ข้อ 4)
⇒ ยืนอยู่บนเครื่องอื่น ให้อ่านตู้ `ALL` **ของเครื่องตัวเอง** ด้วย (`komphet-air` มี `Q-KWS-004` เป็นกติกายืน)
