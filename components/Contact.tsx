...
import { buildOpenChatLink, SOURCE_CONTACT_OPEN_CHAT, SOURCE_CONTACT_QR_CARD } from "@/lib/whatsapp";
    ...
    <section className="pt-8 md:pt-10">
      <Reveal>
        <div className="grid items-end gap-6 lg:grid-cols-[1fr_360px]">
          ...
          <a href={buildOpenChatLink(SOURCE_CONTACT_OPEN_CHAT)} target="_blank" rel="noreferrer" className="btn-shine inline-flex items-center justify-center gap-2 rounded-2xl px-5 py-3.5 text-sm font-semibold"><MessageCircle className="h-4 w-4" /> Start on WhatsApp <ArrowUpRight className="h-4 w-4" /></a>
        </div>
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-12">
        ...
        <Reveal className="lg:col-span-5" delay={0.06}>
          <div className="h-full rounded-[30px] border border-[#d7c39a] bg-[#f5efe3] p-6 text-[#183b2b] shadow-[0_20px_48px_rgba(11,48,29,0.22)] md:p-8">
            ...
            <a href={buildOpenChatLink(SOURCE_CONTACT_QR_CARD)} target="_blank" rel="noreferrer" className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#183d2d] px-5 py-3 text-sm font-semibold text-[#f7f3e9] transition hover:bg-[#25563e]"><MessageCircle className="h-4 w-4 text-[#d7b874]" /> Open WhatsApp <ArrowUpRight className="h-4 w-4 text-[#d7b874]" /></a>
