...
|---|---|---|---|---|---|---|
| W-7 | อัปเดต Node ใน CI (workflow pin `node-version: "20"` · GitHub บังคับรันบน 24 แล้วและขึ้น deprecation annotation ทุก run) | 🟡 queued | — | `.github/workflows/nextjs.yml` | `npm run build && npm run lint` + ดู run จริงว่า annotation หาย | ยังไม่พัง แต่เป็นนาฬิกาที่เดินอยู่ · แตะ deploy path ⇒ ต้องดู run ถัดไปให้ success จริงก่อนถือว่าจบ |
