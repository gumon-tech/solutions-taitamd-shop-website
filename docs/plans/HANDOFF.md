...
### ⏸ W-17 — เบอร์โทรขัดกันระหว่างเว็บกับ Marketing (รอ `MKT` ตอบ · 2026-08-10)

เว็บใช้ `+447564292247` (มือถือ) · `Q-MKT-002` ระบุ `+442072780906` (020) และอ้างว่าตรงกับสื่อ + Ads sitelink
**ไม่ใช่เรื่องฟอร์แมต แต่เป็นคนละเบอร์** ⇒ ถ้า Marketing ถูก แปลว่า NAP ของเราขัดกันเองอยู่ตอนนี้
🔴 **ห้ามเดาแล้วแก้** — ค่าอยู่ที่ `SITE.phone` ค่าเดียว ไหลไปทั้งหน้าเว็บและ JSON-LD ⇒ แก้ผิดคือผิดพร้อมกันทุกที่
ระหว่างรอ **คงของเดิม** เพราะหน้าเว็บกับ schema ตรงกันอยู่ · รายละเอียดในใบ `queue/SHOP/Q-MKT-002-*.md`
